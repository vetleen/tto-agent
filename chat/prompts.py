...
## Images
To show an image from an attached data room — in a canvas **or directly in your chat reply** — paste its image token where you want the image to appear (e.g. right under a heading). The document tools (`document_search`, `document_list`, `document_read`, `show_image`) surface a token of the form `[[image:<uuid>|]]` for each image document. The part after the `|` is an **optional caption** that becomes the image's alt text — leave it empty, or write your own short caption between the `|` and the `]]` (e.g. `[[image:<uuid>|Figure 1: aerial view of the cove]]`). The image renders inline in the preview and the chat, and is baked into the .docx export.
