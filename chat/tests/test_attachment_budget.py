...
class EnrichVectorPageImagesTests(TransactionTestCase):
    """Upload-turn PDFs get rendered vector-page images for OpenAI models only."""

    def setUp(self):
        self.user = User.objects.create_user(email="vec@example.com", password="pw")
        self.thread = ChatThread.objects.create(created_by=self.user)

    async def _run(self, model, provider):
        att = await database_sync_to_async(ChatAttachment.objects.create)(
            thread=self.thread, uploaded_by=self.user,
            file=SimpleUploadedFile("deck.pdf", b"%PDF-1.4\n" + b"x" * 500, content_type=_PDF_MIME),
            original_filename="deck.pdf", content_type=_PDF_MIME, size_bytes=509,
        )
        history = [{"role": "user", "content": "look", "attachment_ids": [str(att.id)]}]
        messages = [Message(role="system", content="sys"), Message(role="user", content="look")]
        ctx = RunContext.create()
        c = ChatConsumer()
        c.scope = {"user": self.user}
        c.user = self.user
        with patch("llm.display.supports_modality", return_value=True), \
             patch("chat.services.detect_provider", return_value=provider), \
             patch("chat.services.get_or_extract_attachment_text", return_value="body"), \
             patch("chat.pdf_attach.pdf_input_misses_vectors", return_value=provider == "openai"), \
             patch("chat.pdf_attach.pages_with_unrendered_vectors", return_value=[1]), \
             patch("chat.pdf_attach.render_pdf_pages_to_jpegs", return_value=([b"jpeg-page-2"], 3)):
            await c._enrich_with_attachments(messages, history, model, ctx, thread_id=str(self.thread.id))
        return messages[1].content, ctx

    async def test_openai_adds_page_images_and_note(self):
        blocks, ctx = await self._run("openai/gpt-6-luna", "openai")
        types = [b.get("type") for b in blocks]
        self.assertIn("file", types)
        images = [b for b in blocks if b.get("type") == "image_url"]
        self.assertEqual(len(images), 1)
        self.assertEqual(images[0]["_wf_pathway"], "attachment")
        self.assertTrue(any("rendered images of page 2 included" in b.get("text", "") for b in blocks))
        self.assertGreater(ctx._b64_used_by_pathway.get("attachment", 0), 0)

    async def test_anthropic_adds_no_page_images(self):
        blocks, _ = await self._run("anthropic/claude-haiku-4-5", "anthropic")
        self.assertIn("document", [b.get("type") for b in blocks])
        self.assertFalse(any(b.get("type") in ("image", "image_url") for b in blocks))
